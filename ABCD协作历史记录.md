...
- 改动文件：
  ...
  - `src/main/resources/styles/app.css`
- 完成内容：
  - 专门修登录页三张角色入口卡的文字出界问题
  - 卡片正文补了固定安全区，不再让标题和副标题贴着左上角
  - 标题、副标题统一改成居中排版，并把卡片高度抬高到更安全的区间
- 影响提醒：
  - 这次动的是登录页入口卡片的布局和样式，A 区域后续如果继续重做登录页，先以这版安全区为基线
  - 当前桌面端已重新启动验证，必须看新窗口，不要用旧登录页截图判断
- Push 结果：
  - 本次 A 改动将随当前这轮提交一起推送
  - 提交号：以本次 push 后的最新提交为准
- 下一步：
